...
public class SplineInterpolator : MonoBehaviour
{
    ...
    {
        return mState != "Reset" && mState != "Stopped";
    }
	...
	void Update()
	{
		...
		if (mState != "Stopped")
		{
            ...
            lastTangent = GetHermiteTangent(mCurrentIdx, param);
	...
	public Vector3 GetHermiteTangent(int idxFirstPoint, float t)
	{
		float t2 = t * t;
		float t3 = t2 * t;
        ...
        return (3 * T2 + 3 * T1 - 6 * P2 + 6 * P1) * t2 + (-2 * T2 - 4 * T1 + 6 * P2 - 6 * P1) * t + T1;
	}
